...
import {
  ...
  renderRecipe,
  runCreate,
} from '../src/orchestration/create.js';
...
test('creates a minimal Svelte project without installing dependencies', async () => {
  ...
  assert.match(
    readme,
    new RegExp(`leftium@${version.replaceAll('.', '\\.')} create`),
  );
...
test('validates inputs and rejects symbolic destinations before scaffolding', async () => {
  ...
  await assert.rejects(
    () =>
      runCreate({ cwd: invalid, types: 'invalid' as never, install: false }),
    /Unsupported --types/,
  );
  ...
  await assert.rejects(
    () => readFile(join(invalidLicense, 'package.json')),
    /ENOENT/,
  );
  ...
  await assert.rejects(
    () => runCreate({ cwd: link, install: false }),
    /symbolic link/,
  );
...
test('composes the official Prettier add-on and the Leftium license add-on', async () => {
  ...
  assert.match(
    await readFile(join(cwd, 'LICENSE'), 'utf8'),
...
test('replays the recorded recipe through the packed local CLI', async () => {
  ...
  await runCreate({
    cwd: first,
    addons: ['prettier', 'license'],
    author: "Ada Lovelace's Workshop",
    year: '1843',
    install: false,
  });
  ...
  assert.ok(command);
  const expectedArgv = recreationRecipeArgv({
    cwd: first,
    template: 'minimal',
    types: 'typescript',
    addons: ['prettier', 'license'],
  ...
  for (const project of [first, second]) {
    assert.match(
      await readFile(join(project, 'src/routes/+page.svelte'), 'utf8'),
      /Welcome to SvelteKit/,
    );
    assert.match(
      await readFile(join(project, 'src/app.d.ts'), 'utf8'),
      /declare global/,
    );
    assert.match(
    ...
    assert.match(
      ...
      /Copyright \(c\) 1843 Ada Lovelace's Workshop/,
    );
    const readme = await readFile(join(project, 'README.md'), 'utf8');
    assert.equal(
      (readme.match(/<!-- leftium:creation-recipe -->/g) ?? []).length,
      1,
    );
    assert.doesNotMatch(readme, /npx sv create my-app/);
