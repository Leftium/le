...
export async function readRegular(path: string): Promise<string | undefined> {
  try {
    ...
    if (!stat.isFile() || stat.nlink > 1) {
      throw new Error(`Expected an ordinary file without symlinks or hard links: ${path}`);
    }
...
export type Mutation = { path: string; before: string | undefined; after: string };

export async function applyEdits(edits: Mutation[], changed: string[]): Promise<void> {
  ...
  for (const edit of edits) {
    const flags = edit.before === undefined
      ? constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL
      : constants.O_RDWR | constants.O_NOFOLLOW;
    const handle = await open(edit.path, flags, 0o666);
    try {
      if (edit.before !== undefined) {
        if ((await handle.stat()).nlink > 1 || await handle.readFile('utf8') !== edit.before) {
          throw new Error(`File changed during planning; retry: ${edit.path}`);
        }
      }
      ...
      while (offset < content.length) {
        ...
        if (!bytesWritten) throw new Error(`Could not finish writing ${edit.path}`);
        offset += bytesWritten;
      }
