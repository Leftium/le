...
export type Manifest = { path: string; text: string; data: Record<string, unknown> };
...
export function parseManifest(text: string, path: string): Manifest {
  ...
  if (!tree || tree.type !== 'object' || errors.length) throw new Error(`Invalid package.json: ${path}`);
  function rejectDuplicates(node: Node): void {
    if (node.type === 'object') {
      const names = node.children?.map(property => property.children?.[0]?.value);
      if (names && new Set(names).size !== names.length) throw new Error(`Duplicate package.json keys: ${path}`);
    }
...
function workspacePatterns(data: unknown): string[] | undefined {
  const patterns = Array.isArray(data) ? data :
    data && typeof data === 'object' && 'packages' in data ? data.packages : undefined;
  return Array.isArray(patterns) && patterns.every(value => typeof value === 'string') ? patterns : undefined;
}

function includesPackage(root: string, member: string, patterns: string[]): boolean {
  const path = relative(root, member).split('\\').join('/');
  return patterns.filter(p => !p.startsWith('!')).some(p => minimatch(path, p)) &&
    !patterns.filter(p => p.startsWith('!')).some(p => minimatch(path, p.slice(1)));
}
...
export async function discover(cwd: string): Promise<ProjectContext> {
  ...
  for (let directory = target; ; directory = dirname(directory)) {
    ...
    if (text !== undefined) {
      ...
      else { try { manifest = parseManifest(text, path); } catch { /* No inference. */ } }
    ...
    if (!context.workspaceRoot) {
      ...
      if (yaml !== undefined) {
        ...
        if (document.errors.length === 0) patterns = workspacePatterns(document.toJS());
      }
      if (patterns && (directory === target || (context.packageRoot && includesPackage(directory, context.packageRoot, patterns)))) {
