...
export function createCommand(): Command {
  ...
  program
    ...
    .option(
      '--force',
      'replace conflicting license content and package license metadata',
    )
    .option(
      '--package-license',
      'explicitly add a separate license at the selected workspace package',
    )
    .option(
      '--non-interactive',
      ...
      async (
        ...
        if (!addon) {
          ...
          if (prompts.isCancel(choice)) {
            process.stderr.write('canceled: Canceled before writing.\n');
            process.exitCode = 1;
            return;
          }
        ...
        if (tty && addon === 'gitattributes' && options.preset === undefined) {
          const choice = await prompts.multiselect({
            message: 'Select git attribute presets',
            required: true,
            initialValues: ['nodiff'],
            options: [
              { value: 'nodiff', label: 'nodiff' },
              { value: 'eol', label: 'eol' },
            ],
          });
          if (prompts.isCancel(choice)) {
  ...
  program
    ...
    .option(
      '--add <addon>',
      'creation add-on (prettier or license)',
    ...
    .action(async (directory: string, options) => {
      try {
        ...
        process.stdout.write(
          `created: ${result.cwd}${result.installed ? '' : ' (dependencies not installed)'}\n`,
        );
        if (result.warning)
